...
export function ProjectDetailView({
  ...
  // Prepare images array for lightbox (WITHOUT LONG DESCRIPTIONS AS REQUESTED)
  ...
  return (
    <main className="pt-24 sm:pt-28 pb-20 w-full min-h-screen bg-zinc-950 text-white transition-colors duration-300">
      ...
      <div className="w-full px-4 sm:px-8 md:px-12 lg:px-16 xl:px-20 2xl:px-28 mb-8">
        <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-zinc-800">
          ...
          <Link
            href="/proyectos"
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-black bg-zinc-900 hover:bg-zinc-800 text-white border border-zinc-750 transition-all cursor-pointer shadow-sm active:scale-95"
          >
      ...
      <div className="w-full px-4 sm:px-8 md:px-12 lg:px-16 xl:px-20 2xl:px-28">
        ...
        <div className="max-w-4xl mb-10 space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="px-3.5 py-1 rounded-full bg-white text-zinc-950 text-xs font-black uppercase tracking-wider shadow-sm">
              {project.categoryLabel}
            </span>
            {project.featured && (
              <span className="px-3 py-1 rounded-full bg-zinc-900 border border-zinc-800 text-zinc-200 text-xs font-bold flex items-center gap-1.5">
          ...
          {/* Title and Transparent Vector Logo (SIZES OPTIMIZED & PROPORTIONAL) */}
          <div className="flex flex-wrap items-center gap-4">
            {project.hasCustomLogo && isDidusa && (
              <div className="py-1 inline-flex items-center justify-center">
                <DidusaLogo className="h-10 sm:h-14 w-32 sm:w-44 text-white" />
              </div>
            )}
            {project.hasCustomLogo && isDecora && (
              <div className="py-1 inline-flex items-center justify-center">
                <DecoraGroupLogo className="h-10 sm:h-14 w-36 sm:w-48 text-white" />
              </div>
            )}
            ...
            {project.hasCustomLogo && isClubMed && (
              <div className="py-1 inline-flex items-center justify-center">
                <ClubMedLogo className="h-9 sm:h-12 w-auto text-white" />
              </div>
            )}
            ...
            {project.hasCustomLogo && isCami && (
              <div className="py-1 inline-flex items-center justify-center">
                <CamiLogo className="h-10 sm:h-14 w-auto text-white" />
              </div>
            )}
            {project.hasCustomLogo && isFancyRd && (
              <div className="py-1 inline-flex items-center justify-center">
                <FancyRdLogo className="h-10 sm:h-14 w-auto text-white" />
              </div>
            )}
            {project.hasCustomLogo && isFacturaDo && (
              <div className="py-1 inline-flex items-center justify-center">
                <FacturaDoLogo className="h-8 sm:h-12 w-auto text-white" />
              </div>
            )}
          ...
          <p className="text-base sm:text-xl text-zinc-300 font-semibold leading-relaxed">
            {project.subtitle}
          </p>

          {/* Client & Metadata Strip */}
          <div className="flex flex-wrap items-center gap-6 pt-2 text-xs sm:text-sm text-zinc-400">
            <div className="flex items-center gap-2">
              <User className="w-4 h-4 text-indigo-400" />
              <span>Cliente: <strong className="text-white font-bold">{project.client}</strong></span>
            </div>
            {project.websiteUrl && (
              <a 
            ...
            {project.instagramUrl && (
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer" 
                className="flex items-center gap-2 text-pink-400 hover:underline font-bold"
              >
        ...
        {isFancyRd && fancyLogos.length > 0 && (
          <div className="mb-14 space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="text-xs text-zinc-400 font-medium">Haz clic en cualquier versión para ampliar en alta definición</span>
            </div>
              ...
              {fancyLogos.map((img, idx) => {
                ...
                return (
                  <div 
                    ...
                    className="group rounded-2xl overflow-hidden border border-zinc-800 bg-zinc-900/80 hover:bg-zinc-900 shadow-xl transition-all duration-300 hover:border-pink-500/50 cursor-pointer flex flex-col"
                  >
                    <div className="relative w-full aspect-square bg-zinc-950 flex items-center justify-center p-6 overflow-hidden">
                      <Image
                        src={img.src}
                        alt={img.alt}
                        fill
                      ...
                      <div className="absolute top-2.5 left-2.5 z-10">
                        <span className="px-2 py-0.5 rounded-md bg-zinc-900/90 backdrop-blur-md border border-zinc-800 text-[10px] font-mono font-bold text-pink-300 shadow-sm">
                          Variante #{idx + 1}
                        </span>
                      </div>
                    ...
                    <div className="p-3 bg-zinc-900 border-t border-zinc-800 flex items-center justify-between text-xs">
                      ...
                      <span className="text-[10px] font-bold text-pink-400 shrink-0">Logo & Marca</span>
                    </div>
                  </div>
                );
              })}
        ...
        {/* 2. CLUB MED MERCHANDISING COLLECTION (16 Colors Grid - CLEAN & NO DESCRIPTIONS) */}
        {isClubMed && galleryImages.length > 0 && (
          <div className="mb-14 space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="text-xs text-zinc-400 font-medium">Haz clic para ampliar en alta resolución</span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
              {galleryImages.map((img, idx) => (
                <div 
                  ...
                  className="group rounded-2xl overflow-hidden border border-zinc-800 bg-zinc-900/80 hover:bg-zinc-900 shadow-xl transition-all duration-300 hover:border-sky-500/50 cursor-pointer flex flex-col"
                >
                  <div className="relative w-full aspect-square bg-zinc-950 flex items-center justify-center p-3 overflow-hidden">
                    <Image
                      src={img.src}
                      alt={img.alt}
                      fill
                    ...
                    {/* Variant Badge */}
                    <div className="absolute top-2.5 left-2.5 z-10">
                      <span className="px-2 py-0.5 rounded-md bg-zinc-900/90 backdrop-blur-md border border-zinc-800 text-[10px] font-mono font-bold text-sky-300 shadow-sm">
                        Color #{idx + 1}
                      </span>
                    </div>
                  ...
                  <div className="p-3 bg-zinc-900 border-t border-zinc-800 flex items-center justify-between text-xs">
                    ...
                    <span className="text-[10px] font-bold text-sky-400">Bordado 3D</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* 3. BROCHURE & EDITORIAL CATALOG VIEW (Big Print 2-Page Presentation) */}
        {isBigPrint && galleryImages.length > 0 && (
          <div className="mb-14 space-y-6">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-400 font-medium">Haz clic para ampliar en alta definición</span>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
              {galleryImages.map((img, idx) => (
                <div 
                  key={idx} 
                  onClick={() => openLightbox(idx)}
                  className="group rounded-3xl overflow-hidden border border-zinc-800/80 bg-zinc-900/60 hover:bg-zinc-900 shadow-2xl transition-all duration-300 hover:border-yellow-500/50 cursor-pointer flex flex-col"
                >
                  <div className="relative w-full aspect-[2376/1836] bg-zinc-950 flex items-center justify-center p-3 sm:p-5 overflow-hidden">
                    <Image
                      src={img.src}
                      alt={img.alt}
                      fill
                    ...
                    <div className="absolute top-4 left-4 z-10">
                      <span className="px-3 py-1 rounded-full bg-zinc-900/90 backdrop-blur-md border border-zinc-800 text-[11px] font-bold text-zinc-200 shadow-sm flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-full ${idx === 0 ? 'bg-yellow-400' : 'bg-emerald-400'}`} />
                  ...
                  {/* Clean Bottom Bar (NO DESCRIPTIONS) */}
                  <div className="p-4 bg-zinc-900/90 border-t border-zinc-800 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 transition-colors shrink-0 ml-3"
        ...
        {/* 4. CAMI INSTITUTO SOCIAL MEDIA FLYERS (14 Unique Pieces Square Grid - CLEAN & NO DESCRIPTIONS) */}
        {isCami && galleryImages.length > 0 && (
          <div className="mb-14 space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="text-xs text-zinc-400 font-medium">Haz clic en cualquier pieza para ampliar en alta definición</span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
              {galleryImages.map((img, idx) => (
                <div 
                  key={idx} 
                  onClick={() => openLightbox(idx)}
                  className="group rounded-2xl overflow-hidden border border-zinc-800 bg-zinc-900/80 hover:bg-zinc-900 shadow-xl transition-all duration-300 hover:border-rose-500/50 cursor-pointer flex flex-col"
                >
                  <div className="relative w-full aspect-square bg-zinc-950 flex items-center justify-center p-2 overflow-hidden">
                    <Image
                      src={img.src}
                      alt={img.alt}
                      fill
                    ...
                    {/* Badge */}
                    <div className="absolute top-2.5 left-2.5 z-10">
                      <span className="px-2 py-0.5 rounded-md bg-zinc-900/90 backdrop-blur-md border border-zinc-800 text-[10px] font-mono font-bold text-rose-300 shadow-sm">
                        Flyer #{idx + 1}
                      </span>
                    </div>
                  ...
                  <div className="p-3 bg-zinc-900 border-t border-zinc-800 flex items-center justify-between text-xs gap-2">
                    <span className="font-bold text-zinc-200 group-hover:text-white truncate">
                      {(img.title || '').replace('CAMI Instituto — ', '')}
                    </span>
                    <span className="text-[10px] font-bold text-rose-400 shrink-0">Social Media</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* 5. BRANDING & LOGO PROJECTS VIEW (Clean Logo Studio Gallery - NO DESCRIPTIONS) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              {galleryImages.map((img, idx) => (
                <div 
                  key={idx} 
                  onClick={() => openLightbox(idx)}
                  className="group rounded-3xl overflow-hidden border border-zinc-800/80 bg-zinc-900/60 hover:bg-zinc-900 shadow-2xl transition-all duration-300 hover:border-zinc-700 cursor-pointer flex flex-col"
                >
                  {/* Image Card Box */}
                  <div className="relative w-full aspect-square sm:aspect-[4/3] bg-zinc-950 flex items-center justify-center p-6 sm:p-8 overflow-hidden">
                    <Image
                      src={img.src}
                      alt={img.alt}
                      fill
                    ...
                    {/* View Badge */}
                    <div className="absolute top-4 left-4 z-10">
                      <span className="px-3 py-1 rounded-full bg-zinc-900/90 backdrop-blur-md border border-zinc-800 text-[11px] font-bold text-zinc-200 shadow-sm">
                  ...
                  {/* Card Bottom Meta (CLEAN & NO DESCRIPTIONS) */}
        ...
        {/* 6. BUSINESS CARDS & PRINT PROJECTS VIEW (Clean 2-Photo Showcase Gallery - NO DESCRIPTIONS) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              {galleryImages.map((img, idx) => (
                ...
                >
                  ...
                  <div className="relative w-full aspect-[1050/600] bg-zinc-950 flex items-center justify-center p-4 sm:p-6 overflow-hidden">
                    <Image
                      src={img.src}
                      alt={img.alt}
                      fill
                    ...
                    <div className="absolute top-3.5 left-3.5 z-10">
                      <span className="px-3 py-1 rounded-full bg-zinc-900/90 backdrop-blur-md border border-zinc-800 text-[11px] font-bold text-zinc-200 shadow-sm flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-full ${idx === 0 ? 'bg-emerald-400' : 'bg-indigo-400'}`} />
        ...
        {project.stats && (
          <div className="mb-12 grid grid-cols-2 md:grid-cols-3 gap-4">
            {project.stats.map((s, idx) => (
              <div key={idx} className="p-5 rounded-2xl bg-zinc-900 border border-zinc-800 shadow-md text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-14">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Overview */}
            <div className="p-6 sm:p-8 rounded-3xl bg-zinc-900 border border-zinc-800 shadow-md space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-6 rounded-3xl bg-zinc-900 border border-zinc-800 shadow-md space-y-3">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-950/80 border border-red-800/60 text-red-300 text-[11px] font-bold uppercase tracking-wider">
                  El Desafío
                </div>
              ...
              <div className="p-6 rounded-3xl bg-zinc-900 border border-zinc-800 shadow-md space-y-3">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-800/60 text-emerald-300 text-[11px] font-bold uppercase tracking-wider">
            ...
            <div className="p-6 sm:p-8 rounded-3xl bg-zinc-900 border border-zinc-800 shadow-md space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {project.deliverables.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-3 p-4 rounded-2xl bg-zinc-800/90 hover:bg-zinc-750 text-xs sm:text-sm font-bold text-white border border-zinc-700/70 shadow-xs transition-colors"
                  >
          ...
          {/* Right Column: Meta Specs & Contact Card (NO COLOR PALETTE) */}
          <div className="space-y-6">
            ...
            <div className="p-6 rounded-3xl bg-zinc-900 border border-zinc-800 shadow-md space-y-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {project.tools.map((tool, idx) => (
                  <span
                    key={idx}
                    className="text-xs font-bold px-3 py-1 rounded-full bg-zinc-800 text-zinc-200 border border-zinc-700/60"
                  >
                    {tool}
                  </span>
                ))}
            ...
            <div className="p-6 rounded-3xl bg-zinc-900 border border-zinc-800 text-white shadow-xl space-y-4">
              ...
              <div className="flex flex-col gap-2">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-full bg-white text-zinc-950 hover:bg-zinc-200 text-xs font-black shadow-md transition-all cursor-pointer"
                >
                ...
                <Link
                  href="/contacto"
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-full bg-zinc-800 hover:bg-zinc-750 text-white text-xs font-bold border border-zinc-700 transition-all cursor-pointer"
                >
        ...
        {/* Related Projects Section (if provided) */}
        {project.relatedProjects && project.relatedProjects.length > 0 && (
          <div className="mb-14 p-6 sm:p-8 rounded-3xl bg-zinc-900/50 border border-zinc-800">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-indigo-400 shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {project.relatedProjects.map((item, idx) => (
                <div 
                  key={idx}
                  className="p-4 rounded-2xl bg-zinc-900 border border-zinc-800/90 hover:border-zinc-700 transition-all group flex flex-col justify-between"
                >
                  <div className="space-y-2">
                    <span className="px-2.5 py-1 rounded-full bg-zinc-800 text-[10px] font-bold text-indigo-300 uppercase">
                      {item.category}
                    </span>
        ...
        <div className="pt-8 border-t border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          {prevProject ? (
            ...
            >
              ...
              <span>Proyecto Anterior: <strong className="text-white">{prevProject.title}</strong></span>
            </Link>
          ) : (
            <div />
          )}
          ...
          <Link
            href="/proyectos"
            className="px-6 py-2.5 rounded-full bg-zinc-900 hover:bg-zinc-800 text-xs font-black text-white border border-zinc-700 shadow-md transition-colors cursor-pointer"
          >
          ...
          {nextProject ? (
            <Link
              ...
              className="inline-flex items-center gap-2 text-xs font-bold text-zinc-400 hover:text-white transition-colors cursor-pointer"
            >
              <span>Siguiente Proyecto: <strong className="text-white">{nextProject.title}</strong></span>
